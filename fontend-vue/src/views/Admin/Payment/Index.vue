        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-sm-5">
                            <a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#createModalPayment"
                                class="btn btn-danger mb-2"><i class="mdi mdi-plus-circle me-2"></i> Add Payment</a>
                    ...
                    <table class="table table-hover table-striped">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th @click="softBy('name')">
                                    <span class="fs-5 fw-bold text-dark me-2">Card Name</span>
                                ...
                                <th @click="softBy('categories_count')">
                                    <span class="fs-5 fw-bold text-dark me-2">Card Number</span>
                                    <span v-if="config.order_by == 'categories_count'">
                                        <i v-if="config.mode == 'desc'" class="fas fa-sort-down"></i>
                                        <i v-else class="fas fa-sort-up"></i>
                                    </span>
                                    <span v-else>
                        ...
                        <tbody>
                            <tr v-for="Payment in Payments" :key="Payment.id">
                                ...
                                <td>{{ Payment.id }}</td>
                                <td class="sorting">{{ Payment.card_name }}</td>
                                <td class="sorting">{{ Payment.card_number }}</td>
                                <td class="sorting">
                                    <div>
                                        <span class="fw-bold text-dark">Chủ sỏ hữu: </span>
                                        <span>{{ Payment.user_name }}</span>
                                    </div>
                                    <div>
                                        <span class="fw-bold text-dark">Liên hệ: </span>
                                ...
                                <td class="table-action">
                                    <a href="javascript:void(0);" @click="showPayment(Payment.id)"
                                        class="action-icon text-info">
                                        <i class="mdi mdi-eye"></i>
                                    </a>
                                    <a href="javascript:void(0);" @click="editPayment(Payment.id)"
                                        class="action-icon text-success">
                                        <i class="mdi mdi-square-edit-outline"></i>
                                    </a>
                                    <Delete :id="Payment.id" :load-data="loadData" />
                                </td>
                            </tr>
...
export default {
    ...
    methods: {
        ...
        loadData() {
            let url = "http://127.0.0.1:8000/api/admin/payments?page=" + this.config.page
